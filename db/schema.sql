...
alter table properties_sale add column if not exists is_archived boolean not null default false;
alter table properties_sale add column if not exists archived_at timestamptz;
alter table properties_sale add column if not exists archived_by uuid references auth.users(id) on delete set null;
alter table properties_rent add column if not exists is_archived boolean not null default false;
alter table properties_rent add column if not exists archived_at timestamptz;
alter table properties_rent add column if not exists archived_by uuid references auth.users(id) on delete set null;
alter table buyers add column if not exists is_archived boolean not null default false;
alter table buyers add column if not exists archived_at timestamptz;
alter table buyers add column if not exists archived_by uuid references auth.users(id) on delete set null;
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'properties_sale_archive_state_check' and conrelid = 'properties_sale'::regclass
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'properties_rent_archive_state_check' and conrelid = 'properties_rent'::regclass
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'buyers_archive_state_check' and conrelid = 'buyers'::regclass
  ) then
    alter table buyers
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'clients_archive_state_check' and conrelid = 'clients'::regclass
  ) then
    alter table clients
...
create index if not exists idx_properties_sale_active_updated_at on properties_sale(updated_at desc) where is_archived = false;
create index if not exists idx_properties_sale_archived_at on properties_sale(archived_at desc) where is_archived = true;
create index if not exists idx_properties_rent_active_updated_at on properties_rent(updated_at desc) where is_archived = false;
create index if not exists idx_properties_rent_archived_at on properties_rent(archived_at desc) where is_archived = true;
create index if not exists idx_buyers_active_updated_at on buyers(updated_at desc) where is_archived = false;
create index if not exists idx_buyers_archived_at on buyers(archived_at desc) where is_archived = true;
create index if not exists idx_clients_active_updated_at on clients(updated_at desc) where is_archived = false;
create index if not exists idx_clients_archived_at on clients(archived_at desc) where is_archived = true;
