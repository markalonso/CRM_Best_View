...
export function HierarchyManager() {
  ...
  const [resetOverrideTarget, setResetOverrideTarget] = useState<EffectiveFieldDefinition | null>(null);
    ...
    try {
      ...
      setNotice(
        !editingField && fieldScopeInput === "selected_node"
          ? `${result.field.default_label || fieldLabelInput.trim()} created for ${selectedNode?.name || "the selected node"} via custom-value storage.`
  ...
  return (
    ...
    <section className="space-y-4">
                ...
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                  <div className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <button disabled={saving || selectedNode.is_root} onClick={() => setArchiveModalOpen(true)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-40">
                        ...
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {(field.is_system || field.storage_kind === "core_column") && (
                                <span className={`rounded px-2 py-0.5 text-[10px] font-semibold uppercase ${selectedForEdit ? "bg-white/15 text-white" : "bg-orange-100 text-orange-800"}`}>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {overrideApplied && selectedNode && (
                              <button onClick={() => setResetOverrideTarget(field)} disabled={saving} className={`rounded-lg border px-3 py-2 text-xs font-medium ${selectedForEdit ? "border-white/30 text-white hover:bg-white/10" : "border-blue-300 text-blue-700 hover:bg-blue-50"}`}>
                                Reset override
                              </button>
                            )}
                            <button
                              ...
                              className={`rounded-lg border px-3 py-2 text-xs font-medium ${
                                field.is_system || field.storage_kind === "core_column"
                                  ? selectedForEdit
                                    ? "border-amber-300 text-white hover:bg-white/10"
                                    : "border-amber-300 text-amber-800 hover:bg-amber-50"
                                  : selectedForEdit
                                    ? "border-rose-300 text-white hover:bg-white/10"
                                    : "border-rose-300 text-rose-700 hover:bg-rose-50"
                              }`}
                            >
                              {field.is_system || field.storage_kind === "core_column" ? "Delete policy" : "Delete"}
                            </button>
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-[11px] text-slate-500">
                        {editingField
                          ...
                          : "New admin-created hierarchy fields use the custom-value storage path. Creating new physical core columns is disabled here."}
                      </p>
                    </div>
                    ...
                    <div className="mt-4 grid gap-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {hasNodeOverride(editingField, selectedNode.id) && (
                          <button onClick={() => setResetOverrideTarget(editingField)} disabled={saving} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-40">
                            Reset override
                          </button>
                        )}
                      </div>
      ...
      <ConfirmationModal
        open={archiveModalOpen && Boolean(selectedNode)}
        title={`${selectedNode?.is_active ? "Archive" : "Restore"} ${selectedNode?.name || "node"}?`}
        description={
          selectedNode?.is_active
            ? "Archiving keeps the hierarchy node and its configuration, but archived nodes cannot receive new records or media."
            : "Restoring reactivates this hierarchy node so it can be used again."
        }
        impacts={
          selectedNode?.is_active
            ? [
                "The node remains in the hierarchy.",
                "New record assignment is blocked while archived.",
                "New media assignment is blocked while archived."
              ]
            : ["The node becomes active and assignable again."]
        }
        confirmLabel={selectedNode?.is_active ? "Archive node" : "Restore node"}
        confirmDisabled={saving}
        confirming={saving}
        tone={selectedNode?.is_active ? "danger" : "default"}
      ...
      <ConfirmationModal
        ...
        description={
          deleteFieldImpact?.hard_delete_allowed === false
            ? "This field definition is protected from hard delete."
            : "This permanently deletes the field definition and cascades to its stored custom values and node overrides."
        }
        impacts={
          deleteFieldLoading
            ? ["Loading field delete impact…"]
            : [
                `Storage kind: ${deleteFieldTarget?.storage_kind || "-"}`,
                `System field: ${deleteFieldTarget?.is_system ? "yes" : "no"}`,
                `Stored custom values: ${deleteFieldImpact?.custom_value_count || 0}`,
                `Node overrides: ${deleteFieldImpact?.override_count || 0}`,
                deleteFieldImpact?.hard_delete_allowed === false
                  ? deleteFieldImpact?.protection_reason || "This field cannot be hard deleted."
                  : "Hard delete is allowed for this field.",
                deleteFieldImpact?.recommended_action || "Loading delete policy…"
              ]
        }
        confirmLabel={deleteFieldImpact?.hard_delete_allowed === false ? "Protected field" : "Delete field"}
        confirming={saving}
        confirmDisabled={deleteFieldLoading || deleteFieldImpact?.hard_delete_allowed === false}
        onClose={() => {
          setDeleteFieldTarget(null);
          setDeleteFieldImpact(null);
        }}
      ...
      />
      <ConfirmationModal
        open={Boolean(resetOverrideTarget && selectedNode)}
        title={`Reset override for ${resetOverrideTarget?.default_label || "field"}?`}
        description="This removes only the node-specific override and keeps the base field definition intact."
        impacts={[
          `Node: ${selectedNode?.name || "-"}`,
          "Field values and the shared field definition are preserved.",
          "The node will fall back to the inherited/default field behavior after reset."
        ]}
        confirmLabel="Reset override"
        confirming={saving}
        onClose={() => setResetOverrideTarget(null)}
        onConfirm={async () => {
          if (!resetOverrideTarget) return;
          await handleResetOverride(resetOverrideTarget);
        }}
      />
    </>
  );
