...
const AUTHENTICATED_ROLE_FALLBACK: AppRole = "agent";
...
export async function getRequestActor(request: NextRequest): Promise<RequestActor> {
  ...
  const role = (profile?.role || AUTHENTICATED_ROLE_FALLBACK) as AppRole;
  return {
    userId: user.id,
    role: role === "admin" || role === "agent" || role === "viewer" ? role : AUTHENTICATED_ROLE_FALLBACK,
