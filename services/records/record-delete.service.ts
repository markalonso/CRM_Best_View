...
async function deleteRecordsInTransaction(input: { type: GridType; recordIds: string[] }): Promise<DeleteTransactionResult> {
  ...
  if (error) {
    ...
    if (message.toLowerCase().includes("could not find the function public.delete_crm_records_transactional")) {
      throw new Error("Record delete RPC is not available in this environment yet. Apply migrations (including 20260321_record_delete_hardening.sql and 20260401_record_delete_rpc_compat.sql) and refresh the Supabase/PostgREST schema cache.");
    }
    throw new Error(message || "Record delete RPC failed.");
  }
